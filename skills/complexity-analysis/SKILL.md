...
### Lower-bound reasoning tactics (when "can we do better?" comes up)
- **Information-theoretic counting**: an algorithm distinguishing among M possible answers via binary decisions needs ≥ log₂ M of them. Sorting: M = n! → log₂(n!) ≈ n log₂ n − 1.44n. Finding a max: only n−1 comparisons needed and required (each non-winner must lose at least once — a counting argument on its own).
- **Adversary argument**: imagine an adversary who answers queries so as to keep ≥ 2 consistent worlds alive as long as possible. Unsorted search: after k < n probes, the adversary keeps the target in an unprobed cell → Ω(n). Useful reflex for "can I skip reading part of the input?" — only if an invariant (sortedness, an index, a digest) already rules those worlds out.
- **Output-size bound**: enumeration problems are Ω(#answers); counting problems are not (count without enumerating — DP, inclusion-exclusion, determinants).
- **Reduction transfer**: if problem A with a known Ω-bound reduces in O(small) to your problem B, B inherits the bound. Element-distinctness is Ω(n log n) in the comparison model → so is "are there duplicate line segments/pairs", which is why hash-based dedup (different model) is the escape.
- Practical use: a lower-bound argument's real deliverable is redirecting effort — from "make the comparator faster" to "change the model" (index, hash, bit-parallelism, randomization, approximation).
...
**4. Crossover measured, not asserted.** Membership testing, m queries against n items: list scan is O(m·n) with perfect cache behavior; set is O(m) expected with a hashing constant. Microbenchmark shape (`timeit`, n = 5): at n ≈ 5 the list often *wins* (no hash, contiguous scan of a handful of pointers); by n = 100 the set wins ~10×; by n = 10⁵ it's ~10⁴×. The expert habit is the two-point measurement to locate the crossover for *your* value types (hashing a 3-char string vs a 200-field dataclass moves it by an order of magnitude), then choosing — not defaulting to the asymptotically superior structure at n = 4 in an inner loop, and not defending a scan at n = 10⁶.

**5. NP-hard recognition redirect, end-to-end.** "Assign 200 tasks to 20 workers; each task has a qualified-worker subset; some task pairs conflict (can't share a worker); minimize max load." Conflicts + makespan = coloring/scheduling hybrid → NP-hard; do not promise an efficient exact algorithm. Route: CP-SAT model — booleans x[t,w] restricted to qualified pairs, `AddExactlyOne` per task, per conflict pair and worker at most one, minimize the max of worker loads — solves this size in seconds with optimality proof or gap. Fallback if a solver can't be a dependency: greedy (order tasks by fewest qualified workers, assign to least-loaded feasible worker) + swap/2-move local search, validated against CP-SAT on 30-task instances. Hardness spotted → solver-or-heuristic chosen → guarantee stated: that routing is the expert deliverable.
...
- **Feasibility arithmetic before design**: (state count) × (work per state) against the ~10⁸–10⁹ ops/sec anchor; if the product says 3 hours and the budget is 3 seconds, no constant-factor tuning closes 3–4 orders of magnitude — the algorithm class must change.
- **Space audit alongside time**: an O(n) time solution with an O(n²) memo table dies of memory first; check both axes, and check peak (including temporaries from slicing/sorting copies), not just resident state.
- **Adversarial-input probe**: feed sorted, reverse-sorted, all-equal, and pathological-key inputs to anything whose expected-case analysis relies on randomness or hashing; the worst case should degrade gracefully or be explicitly documented as out of contract.
