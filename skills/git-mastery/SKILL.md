...
- **SHA-256:** Git 2.51+ defaults *new local* repos to SHA-256 and Git 3.0 (targeted late 2026) doubles down — but **GitHub still doesn't host SHA-256 repos** and there's no in-place migration or interop bridge. Anything destined for GitHub: `git init --object-format=sha1` if your git defaults to sha256. Check hosting before adopting sha256/reftable defaults.
