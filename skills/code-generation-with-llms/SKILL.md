...
- **Audit machine-written memory.** As of 2026, Claude Code also maintains *auto memory* — notes it writes for itself per repository (`~/.claude/projects/<project>/memory/`, with a `MEMORY.md` index loaded each session), on by default. It compounds the same way your curated files do, and it can also entrench a mistaken inference. It's plain markdown: skim it occasionally (`/memory`), correct wrong learnings, promote good ones into the shared CLAUDE.md so the team gets them too.
- **Promote repeatable workflows into skills.** A procedure you paste into chat more than twice ("here's how we cut a release", "here's how to fix a GH issue end-to-end") belongs in a skill file with a description trigger — loaded on demand, versioned with the repo, and usable by CI agents. Skills are the multi-step counterpart to memory-file one-liners; putting procedures in CLAUDE.md instead bloats every session with steps that rarely apply.
...
- **Skim-approving the plan.** Plan mode only pays off if the plan gets real review — it is the highest-leverage review point in the whole workflow, the one place a wrong approach costs a paragraph to fix. Approving a plan you skimmed converts plan mode into pure latency: you still get the wrong 40-file diff, just later. Edit the plan (cut steps, tighten scope, add the verification step) before releasing the agent to implement.
- **Permission fatigue → rubber-stamping.** After the tenth approval prompt you're clicking, not reviewing — the gate is theater. Fix structurally, not with willpower: allowlist the genuinely safe commands (`npm run lint`, `git commit`), sandbox or scope credentials for the rest, and save human attention for the few actions that are actually irreversible. A gate everyone clicks through is worse than no gate, because it *feels* like control.
