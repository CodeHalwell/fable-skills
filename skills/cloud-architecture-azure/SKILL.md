...
6. **Consumption vs dedicated reasoning** (applies to Functions and ACA alike): consumption billing wins below roughly 30–40% sustained utilization; above that, dedicated capacity (App Service plan, ACA dedicated profiles/workload profiles, AKS with reservations) is cheaper. Compute the duty cycle; don't vibe it.
...
- **Renamed-service drift in generated code:** referencing `Microsoft.Insights` classic APIs, retired PostgreSQL Single Server, or "Azure Active Directory" endpoints in new designs. Verify names against current docs (Microsoft Learn) whenever the training-data name is more than a year old.
