...
- **Compiling the module per invocation server-side**: `Module::from_file` / `Module::new` runs a full compile — doing it per request turns a 2 ms plugin call into a 200 ms one. Correction: compile once, cache the `Module` (or ship Wasmtime-precompiled artifacts); create only the cheap `Store`/instance per request.
- **Missing host capabilities discovered at runtime**: wasm has no ambient clock, entropy, or filesystem — code that needs randomness or time traps or returns constants unless the host wires those capabilities (WASI imports server-side; the appropriate JS/browser entropy feature for crates like `getrandom` in browser builds). Correction: enumerate the module's imports at build time (`wasm-tools`), and treat any unexpected import as a build error, not a deploy-time surprise.
