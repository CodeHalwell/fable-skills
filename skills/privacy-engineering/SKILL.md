...
## Residency and breach readiness (the parts engineering must pre-build)

- **Data residency is an architecture property you can't patch in later.** "EU customer data stays in-region" requires region-pinned storage *and* region-aware processing: the primary DB in `eu-west-1` doesn't help if the warehouse, the LLM API, the error tracker, and the support tooling all replicate to the US. Model residency as a per-tenant attribute that storage routing, queue topology, and vendor selection all consult; the legal instruments (SCCs/DPF as of 2026) are counsel's problem, but they only work if the architecture can actually make true statements about where data flows.
- **Breach notification clocks (GDPR: 72 hours to regulator awareness-to-notify) are an engineering requirement wearing a legal costume:** to notify, you must *scope* — which users, which fields, what window. That demands access logging on PII stores (who read what, when), the data map (what was in the store), and retention of those logs longer than your detection lag. A company that can't answer "what did this compromised credential access?" notifies for the worst case — everyone, everything — because it has no evidence for anything narrower.
- Practical pre-builds: audit logging on every tier-1 store (cloud-native: CloudTrail data events, BigQuery INFORMATION_SCHEMA access logs, pgAudit), a rehearsed query for "all data touched by principal P in window W," and a quarterly tabletop that runs the 72-hour clock against a fictional leaked credential.
...
- Aggregation without DP still needs hygiene: suppress small cells (the classic k≥10 rule for published counts), because "3 employees at site X searched for oncologists" is an identification, not a statistic — and beware *differencing* (this week's total minus last week's isolates individuals even when both totals pass the threshold).
...
- **Support tooling as the unmapped superuser.** The internal admin panel shows any user's full record to every support agent, screenshots of it live in Zendesk tickets and Slack threads forever, and none of it is in the data map. Corrections: field-level masking in admin tools by default (reveal-on-click, logged), support-platform retention policies, and the admin panel listed in the data map as what it is — your highest-traffic PII access path.
- **Feature stores and derived ML tables that outlive their sources.** The `user_features` table rebuilt nightly keeps last-known values for deleted users forever because the rebuild only upserts. Derived stores need either tombstone-aware rebuilds or full periodic recomputation from (deletion-respecting) sources — pick one explicitly.
- **Third-party SDKs collecting on their own account.** Every analytics/ads/monitoring SDK embedded in your web or mobile client ships data directly to its vendor — bypassing your logging controls, your consent gate (unless you wire the SDK's consent API to yours), and often your data map. Inventory client-side SDKs as data recipients, gate their initialization on consent state, and prefer server-side/proxied collection where you can filter what leaves.
- **Test/staging environments seeded with production dumps** — now your weakest-security environment holds your realest data, outside every control in this document. Synthetic data or irreversibly masked subsets for non-prod; make the prod-dump path require an exception with an expiry.
