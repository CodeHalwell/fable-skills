...
- **Account linking by unverified email.** "Sign in with X" arrives bearing `email: victim@corp.com`; your code matches it to the existing password account and merges — but some IdPs pass through unverified emails, so an attacker who registers that email at a lax IdP walks into the victim's account. Only auto-link when `email_verified: true` *and* you trust the issuer's verification (or re-verify yourself with a confirmation loop); otherwise treat same-email as a *prompt* to link after fresh authentication, never an automatic merge.
- **Keying users on email instead of `(iss, sub)`.** `sub` is the stable identifier and it's only unique *per issuer*; emails get recycled (corporate addresses reassigned, domains lapse) and changed. Primary key: issuer + subject; email is a mutable, re-verifiable attribute. Systems keyed on email hand accounts to whoever next controls the address.
