...
**Index/store selection (right-size it):**
- < ~100k chunks: exact search (numpy dot products, FAISS flat, `pgvector` without an index or with HNSW) — recall is 100% by construction; don't buy ANN complexity you don't need.
- Larger: ANN (HNSW is the common default) and *measure* recall of ANN vs exact on your gold set — ANN parameters trade recall for speed and the defaults may be silently costing you.
- Already on Postgres and corpus is moderate: `pgvector` beats introducing a new database — transactional consistency between metadata and vectors, one backup story, SQL pre-filtering for free.
- Dedicated vector DBs earn their keep at scale (sharding, high QPS) or when you need their hybrid/filtering features and don't have Postgres/Elasticsearch already. The database choice matters far less than chunking and hybrid+rerank — teams debate stores for weeks and ship 512-token fixed chunks.
...
- **Asymmetric embedding models used symmetrically.** Several strong embedding families (e5, BGE, and instruction-tuned models) require distinct prefixes/instructions for queries vs passages (e.g., `"query: ..."` vs `"passage: ..."`). Omitting them — or applying the query prefix to documents at ingestion — silently costs meaningful recall with no error raised. Check the model card; encode the convention in one shared function used by both the indexer and the query path so they can't diverge.
- **Duplicate/near-duplicate chunks eating the whole top-k.** Five copies of the same boilerplate paragraph retrieved as ranks 1–5 → the generator sees one fact five times and misses the rest. Dedupe at ingestion (content hash + near-dup detection) and diversify at query time (MMR or simple per-document caps like "max 2 chunks per doc in top-k").
- **Conflicting sources handed over without instructions.** Two retrieved chunks disagree (old vs new pricing); the generator picks arbitrarily or blends them into a fact that appears in neither. Give a resolution rule in the prompt ("prefer the document with the latest effective_date; if sources conflict and dates are absent, state the conflict") and surface doc dates/versions in the chunk headers so the rule is executable.
- **Embedding queries at runtime with a different text pipeline than ingestion.** Lowercasing, whitespace normalization, or HTML stripping applied to documents but not queries (or vice versa) shifts vectors enough to hurt. One canonical `normalize(text)` shared by both paths.
- **Latency budget spent in the wrong stage.** Teams cut the reranker "for speed" while the generator produces 800 unneeded output tokens. Typical shape: embed query ~10ms, ANN search ~10–50ms, rerank 100 candidates ~50–300ms, generation 2–10s. Generation dominates; the reranker is nearly free by comparison and usually the best quality-per-millisecond in the stack.
