...
**Continuous batching — what it is and what you still tune:**
- The scheduler admits and retires requests *per decode step*, not per batch: a finishing sequence's slot is refilled immediately, and new requests' prefills are interleaved (chunked) with ongoing decodes. This is why static-batch mental models ("wait to fill a batch of 32") mispredict everything — batch composition changes every step.
- Knobs that still matter: max concurrent sequences (cap it at what KV arithmetic supports, not the default), max tokens per scheduler step / chunked-prefill chunk size (bigger chunks = better prefill throughput, worse ITL interference), and admission/queueing policy at the load balancer.
- Corollary: per-request "batch size" is invisible to the client and varies with load — which is why ITL for the *same request* differs between a quiet and a busy server, and why load-level must be stated with every latency number.
