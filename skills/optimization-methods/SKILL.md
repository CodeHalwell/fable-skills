...
- **Objective/metric mismatch treated as an optimizer bug.** Optimizing a smooth surrogate (cross-entropy) while judging by a non-smooth metric (F1 at a threshold, top-k accuracy, business revenue) means the optimizer can be *succeeding* while the metric stalls. Before touching optimizer knobs, verify the surrogate still correlates with the metric in the current regime; if not, change the surrogate (differentiable relaxations of ranking/threshold metrics exist) or tune the decision threshold separately.
- **Non-differentiable kinks silently mishandled.** ReLU corners are benign, but `abs`, `max`, quantile, and sorting operations inside custom losses give autodiff subgradients whose choice at the kink can stall progress; smooth them (huber for abs, log-sum-exp for max, soft-sort) when the kink is where the optimum lives — which for L1-type losses it always is.
- **Hyperparameter search treated as an afterthought.** Random search beats grid search at equal budget for >2 hyperparameters (grid wastes evaluations on duplicated projections). Search LR on a log scale always; report the best config *and* its neighborhood sensitivity, since a knife-edge optimum won't survive the next data shift.
...
- **Reproduce the claimed optimum from the stated formulation**: re-evaluate the objective from the returned variables independently of the solver's reported objective value; mismatches expose modeling-layer bugs (units, sign conventions, forgotten terms) that solvers cannot see.
- **Scale test**: rerun with all inputs multiplied by 10; if the solution doesn't transform the way the math says it should (scale-equivariant or invariant), a hard-coded constant or tolerance is acting as a hidden constraint.
